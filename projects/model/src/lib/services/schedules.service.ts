...
export abstract class SchedulesService {
  ...
  importSchedules(data: string, billId: number): Observable<ImportReport[]> {
    const requests: Observable<ImportReport>[] = [];
    const errors: string[] = [];
    data.split(IMPORT_LINE_SEPARATOR).forEach((line, index) => {
      const schedule = this.parseSchedule(line, billId);
      if (schedule) {
  ...
  private parseSchedule(text: string, billId: number): Schedule | undefined {
